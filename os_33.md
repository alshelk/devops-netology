...
### 4. Занимают ли зомби-процессы какие-то ресурсы в ОС (CPU, RAM, IO)?
    ...
    Получается что небольшое количество зомби процессов не страшны для ОС, но вот если они 
    плодатся постоянно, то могут занять всю таблицу системных процессов, чем заблокируют 
    выполнение дргих процессов.
...
*Есть ли смысл использовать в bash &&, если применить set -e?*
    ...
    последовательность команд через && (логический AND) - каждая следующая команды будет выполняться
    только после успешного выполнения предыдущей

    $ ping -c 2 mail.ru && cd /tmpr && ls ~/
    PING mail.ru (94.100.180.201) 56(84) bytes of data.
    64 bytes from mail.ru (94.100.180.201): icmp_seq=1 ttl=63 time=58.3 ms
    64 bytes from mail.ru (94.100.180.201): icmp_seq=2 ttl=63 time=56.3 ms

    --- mail.ru ping statistics ---
    2 packets transmitted, 2 received, 0% packet loss, time 1066ms
    rtt min/avg/max/mdev = 56.259/57.278/58.298/1.019 ms
    -bash: cd: /tmpr: No such file or directory


    последовательность команд через ; - каждая следующая команда будет выполнена после выполнения 
    предыдущей, причем не важно как закончилось выполнение предыдущей команды

    $ ping -c 2 mail.ru ; cd /tmpr ; ls ~/
    PING mail.ru (94.100.180.201) 56(84) bytes of data.
    64 bytes from mail.ru (94.100.180.201): icmp_seq=1 ttl=63 time=56.3 ms
    64 bytes from mail.ru (94.100.180.201): icmp_seq=2 ttl=63 time=56.3 ms

    --- mail.ru ping statistics ---
    2 packets transmitted, 2 received, 0% packet loss, time 1002ms
    rtt min/avg/max/mdev = 56.319/56.326/56.333/0.007 ms
    -bash: cd: /tmpr: No such file or directory
    file  temp  test_delet  test_script.py
